...
function loginUser_som(req, res) {
  const { username, password } = req.body;
  Login_som.findOne({ HTNO: username })
    .then((existingUser) => {
...
async function seeUser_som(req, res) {
  try {
    const user = await Login_som.find({ HTNO: req.decoded.username });
...
module.exports = {
  ...
  loginUser_som: loginUser_som,
  seeUser_som: seeUser_som,
};
